...
- [X] T088 [P] Sincronizar índice Meilisearch em criação/atualização/remoção de produto (cross-story) — decisão: a reindexação síncrona já existente em `ProductAdminService` (chamada direta a `reindexar()`/`removeProduct()` em cada mutação, dentro da mesma transação) é mantida como está. Migrar para um `SearchIndexSyncListener` assíncrono via evento RabbitMQ seria uma refatoração arquitetural de alto risco para um "polish" — o comportamento atual funciona, está testado (`ProductAdminContractTest`) e foi revalidado manualmente nesta sessão (produto criado no backoffice aparece no catálogo público imediatamente). Nenhum código criado para esta task.
- [X] T089 [P] Configurar dashboards Prometheus/Grafana para checkout, pagamento, estoque e pedidos — `infra/prometheus/prometheus.yml` (scrape do backend via `host.docker.internal:8082`, já que o backend roda nativamente, não em container) e `infra/grafana/` (datasources provisionados, dashboard `bikeshop-overview.json` com 4 linhas — taxa de requisições/erro 5xx/latência p95 por domínio, usando as métricas HTTP auto-instrumentadas pelo Micrometer). Serviços `prometheus`/`grafana` adicionados ao `docker-compose.yml` (portas 9090/3003). Validado rodando: target do Prometheus `up`, queries retornando dados reais de tráfego gerado na validação do quickstart.
- [X] T090 [P] Configurar pipeline centralizado de logs (ELK/Loki) — Loki + Promtail (mais leve que ELK) em vez de Elasticsearch/Logstash/Kibana. Backend passou a logar em arquivo além de console (`logging.file.name`) — Promtail lê esse arquivo montado do host (não há container do backend para descoberta automática) e envia para o Loki; datasource Loki provisionado no Grafana. Validado rodando: consulta direta ao Loki (`/loki/api/v1/query_range`) retornou logs reais do backend, incluindo uma entrada do próprio Log de Auditoria.
- [X] T091 Rodar validação completa de `quickstart.md` nos 3 cenários — Cenário 1 (catálogo → carrinho → frete → checkout guest) e Cenário 3 (produto → estoque → pedido → cupom, incluindo rejeição de cupom expirado) validados ponta a ponta via navegador nesta sessão; Cenário 2 validado de forma leve (cadastro, perfil) por não ter sido afetado pelas mudanças da Fase 6 além do mecanismo de token (já coberto por `PaymentSecurityContractTest`). Um bug real de checkout apareceu durante a validação (carrinho de testes acumulado com quantidade 8 excedendo o estoque de 4) — não é regressão desta fase, era dado de teste obsolete de sessões anteriores; corrigido esvaziando o carrinho.
- [X] T092 [P] Auditoria de acessibilidade (WCAG) nas UIs de catálogo, checkout, conta e admin — scan não encontrou violações graves (imagens com `alt`, botões com texto/aria-label, labels associados a inputs, cor sempre acompanhada de texto, nenhum `outline-none` removendo foco). Um achado real corrigido: `text-gray-400` sobre fundo branco em `products/[slug]/page.tsx` (especificações técnicas/geometria) tinha contraste de 2.54:1, abaixo do mínimo WCAG AA (4.5:1) — trocado para `text-gray-500` (4.83:1).
- [X] T093 Revisão de hardening de segurança (expiração/refresh de JWT, verificação de assinatura de webhook, cobertura de RBAC) — 4 correções reais: (1) comparação de assinatura/token de webhook trocada de `.equals()` para `MessageDigest.isEqual` (constant-time) nos 3 gateways; (2) bug real corrigido no webhook do Mercado Pago (o manifest da assinatura usava um header `data-id` inexistente, sempre string vazia — corrigido para usar o `id` real extraído do payload); (3) logout/revogação de token via blacklist no Redis (`TokenBlacklistService`, claim `jti`), novo endpoint `POST /auth/logout`, `POST /auth/refresh` passou a recusar refresh token revogado — e o botão "Sair" do frontend, que antes só limpava o `localStorage`, foi conectado ao endpoint (achado durante a validação manual: o hardening não tinha efeito prático sem isso); (4) checagem de ownership em `POST /payments/{orderId}/intents` (pedido de cliente autenticado só aceita intenção criada pelo próprio dono; pedido de convidado continua público). 3 novos testes de contrato (`PaymentSecurityContractTest`).
- [X] T094 [P] Revisão da documentação de API (Springdoc/OpenAPI) frente a `contracts/api-overview.md` — 2 divergências reais corrigidas (`GET /products/{slug}/variants` e `POST /cart/merge` documentados mas inexistentes) e rotas administrativas de leitura que faltavam (`GET /admin/products`, `GET /admin/products/{id}`, `GET /admin/coupons`, `GET /admin/reviews`) adicionadas à tabela; `POST /auth/logout` documentado.
- [X] T095 Validação de performance frente às metas SC-001 a SC-004 de `spec.md` — critérios são de UX/completude, não throughput; validados via cronometragem manual durante o T091 (todos os fluxos completados em segundos, bem dentro dos limites de 3/10 minutos).
